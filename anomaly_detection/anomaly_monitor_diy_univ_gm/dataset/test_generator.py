import pandas as pd
import numpy as np
import argparse
import logging


class ExtremeAnomalyInjector:
    """Inject extreme anomalies into weather data - realistic sensor failures"""
    
    def __init__(self, dataset_path: str, seed=None):
        """
        Initialize the anomaly injector
        
        Args:
            dataset_path: Path to original dataset.csv (will use last 4000 rows)
            seed: Random seed for reproducibility
        """
        if seed is not None:
            np.random.seed(seed)
        
        self.logger = logging.getLogger(__name__)
        self.dataset_path = dataset_path
        
        # Load dataset
        self._load_data()
    
    def _load_data(self):
        """Load the last 4000 rows from dataset and clean it"""
        self.logger.info(f"Loading dataset from {self.dataset_path}")
        
        df = pd.read_csv(self.dataset_path)
        
        # Take last 4000 rows
        if len(df) > 4000:
            df = df.tail(4000).reset_index(drop=True)
            self.logger.info(f"Using last 4000 rows from dataset")
        else:
            self.logger.info(f"Dataset has {len(df)} rows (less than 4000)")
            df = df.reset_index(drop=True)
        
        # Identify feature columns (exclude date and is_anomaly if present)
        self.feature_cols = [col for col in df.columns if col not in ['date', 'is_anomaly']]
        
        # Clean the data - handle NaN, inf, and ensure numeric types
        self.logger.info("Cleaning data...")
        for col in self.feature_cols:
            # Convert to numeric, coercing errors to NaN
            df[col] = pd.to_numeric(df[col], errors='coerce')
            
            # Replace inf/-inf with NaN
            df[col] = df[col].replace([np.inf, -np.inf], np.nan)
            
            # Count NaN values
            nan_count = df[col].isna().sum()
            if nan_count > 0:
                self.logger.warning(f"Column '{col}' has {nan_count} NaN values, filling with median")
                # Fill NaN with median
                median_val = df[col].median()
                df[col] = df[col].fillna(median_val)
            
            # Ensure it's float type
            df[col] = df[col].astype(float)
        
        self.data = df.copy()
        
        # Add is_anomaly column if it doesn't exist, ensure it's integer
        if 'is_anomaly' not in self.data.columns:
            self.data['is_anomaly'] = 0
        else:
            # Clean is_anomaly column - convert to numeric and fill NaN with 0
            self.data['is_anomaly'] = pd.to_numeric(self.data['is_anomaly'], errors='coerce')
            self.data['is_anomaly'] = self.data['is_anomaly'].fillna(0)
        
        # Ensure is_anomaly is integer (0 or 1)
        self.data['is_anomaly'] = self.data['is_anomaly'].astype(int)
        
        # Calculate statistics
        self.feature_stats = {}
        for col in self.feature_cols:
            self.feature_stats[col] = {
                'min': self.data[col].min(),
                'max': self.data[col].max(),
                'mean': self.data[col].mean(),
                'std': self.data[col].std()
            }
        
        self.logger.info(f"Loaded {len(self.data)} rows with {len(self.feature_cols)} features")
        self.logger.info(f"Data types: {self.data[self.feature_cols].dtypes.to_dict()}")
    
    def _generate_extreme_anomaly(self, feature: str) -> float:
        """
        Generate anomalous values for specific weather features
        
        Args:
            feature: Feature name (tt, rh, pp, ws, wd, sr, rr)
            
        Returns:
            Anomalous value for that feature
        """
        stats = self.feature_stats[feature]
        
        # Feature-specific anomalies
        if feature == 'tt':  # Temperature
            # Temperature deviations: ±8-12°C from normal
            if np.random.rand() > 0.5:
                return float(stats['mean'] + np.random.uniform(8, 12))  # Heat spike
            else:
                return float(stats['mean'] - np.random.uniform(8, 12))  # Cold spike
        
        elif feature == 'rh':  # Relative Humidity
            # Extreme humidity: 0-10% (too dry) or 90-100% (too wet)
            if np.random.rand() > 0.5:
                return float(np.random.uniform(90, 100))  # Too wet
            else:
                return float(np.random.uniform(0, 10))    # Too dry
        
        elif feature == 'pp':  # Atmospheric Pressure
            # Pressure anomalies: high (1030+) or low (930-)
            if np.random.rand() > 0.5:
                return float(np.random.uniform(1030, 1040))  # High
            else:
                return float(np.random.uniform(920, 930))    # Low
        
        elif feature == 'ws':  # Wind Speed
            # Wind speed anomaly: 6-12 m/s (elevated but not extreme)
            return float(np.random.uniform(6, 12))
        
        elif feature == 'wd':  # Wind Direction
            # Invalid values to represent sensor failure
            invalid_values = [-999, 0, 999]
            return float(np.random.choice(invalid_values))
        
        elif feature == 'sr':  # Solar Radiation
            # Solar radiation: 0 (night/failure) or 500-900 (midday saturation)
            if np.random.rand() > 0.5:
                return float(np.random.uniform(500, 900))  # Elevated radiation
            else:
                return 0.0  # No radiation / sensor failure
        
        elif feature == 'rr':  # Rainfall
            # Rainfall anomaly: 2-8 mm (notable rain)
            return float(np.random.uniform(2, 8))
        
        else:
            # Default: push to upper range
            return float(stats['mean'] + stats['std'])
    
    def inject_anomalies(self, n_anomalies: int, min_duration: int = 1, 
                        max_duration: int = 10, min_gap: int = 20) -> pd.DataFrame:
        """
        Inject extreme anomalies into the dataset with minimum gap between anomalies
        
        Args:
            n_anomalies: Number of anomalous events to inject
            min_duration: Minimum duration of each anomaly
            max_duration: Maximum duration of each anomaly
            min_gap: Minimum gap (in timesteps) between consecutive anomalies
            
        Returns:
            DataFrame with injected anomalies and is_anomaly column
        """
        df = self.data.copy()
        
        self.logger.info(f"Injecting {n_anomalies} extreme anomalies with duration {min_duration}-{max_duration}")
        self.logger.info(f"Minimum gap between anomalies: {min_gap} timesteps")
        
        # Calculate available positions considering max_duration
        available_indices = list(range(len(df) - max_duration))
        
        if len(available_indices) < n_anomalies:
            n_anomalies = len(available_indices)
            self.logger.warning(f"Reducing anomalies to {n_anomalies} due to space constraints")
        
        # Select non-overlapping anomaly positions with minimum gap
        anomaly_positions = []
        occupied_ranges = []  # Track occupied time ranges (start, end)
        
        max_attempts = n_anomalies * 100  # Prevent infinite loop
        attempts = 0
        
        while len(anomaly_positions) < n_anomalies and attempts < max_attempts:
            attempts += 1
            
            # Randomly select a candidate position
            candidate_pos = np.random.choice(available_indices)
            
            # Determine duration for this anomaly
            duration = np.random.randint(min_duration, max_duration + 1)
            duration = min(duration, len(df) - candidate_pos)
            
            # Calculate the range this anomaly would occupy (including gap)
            candidate_start = candidate_pos
            candidate_end = candidate_pos + duration - 1
            
            # Check if this position conflicts with existing anomalies (including gap)
            is_valid = True
            for occupied_start, occupied_end in occupied_ranges:
                # Check if there's overlap or gap violation
                # Gap before: candidate must end at least min_gap before occupied starts
                # Gap after: candidate must start at least min_gap after occupied ends
                if not (candidate_end + min_gap < occupied_start or 
                        candidate_start > occupied_end + min_gap):
                    is_valid = False
                    break
            
            if is_valid:
                anomaly_positions.append((candidate_pos, duration))
                occupied_ranges.append((candidate_start, candidate_end))
        
        if len(anomaly_positions) < n_anomalies:
            self.logger.warning(f"Could only place {len(anomaly_positions)} anomalies with min_gap={min_gap}")
        
        # Sort by position for cleaner injection
        anomaly_positions = sorted(anomaly_positions, key=lambda x: x[0])
        
        anomaly_indices = set()
        
        # Inject anomalies
        for start_pos, duration in anomaly_positions:
            # Randomly select 1-2 features to corrupt
            n_features = np.random.randint(1, 3)
            features_to_corrupt = np.random.choice(self.feature_cols, n_features, replace=False)
            
            for i in range(start_pos, start_pos + duration):
                # Generate extreme anomalies for selected features
                for feature in features_to_corrupt:
                    anomalous_value = self._generate_extreme_anomaly(feature)
                    df.at[i, feature] = anomalous_value
                
                # Ensure is_anomaly is set as integer
                df.at[i, 'is_anomaly'] = 1
                anomaly_indices.add(i)
        
        # Final data type cleanup
        for col in self.feature_cols:
            df[col] = df[col].astype(float)
        df['is_anomaly'] = df['is_anomaly'].astype(int)
        
        # Verify no NaN values remain
        for col in self.feature_cols + ['is_anomaly']:
            if df[col].isna().any():
                self.logger.error(f"Column '{col}' still has NaN values after injection!")
                # Fill any remaining NaN
                if col == 'is_anomaly':
                    df[col] = df[col].fillna(0).astype(int)
                else:
                    df[col] = df[col].fillna(df[col].median()).astype(float)
        
        # Log statistics
        n_anomaly_samples = len(anomaly_indices)
        anomaly_ratio = n_anomaly_samples / len(df) * 100
        
        self.logger.info(f"Successfully injected {len(anomaly_positions)} anomalous events")
        self.logger.info(f"Total anomalous samples: {n_anomaly_samples} ({anomaly_ratio:.2f}%)")
        
        # Verify minimum gap
        if len(anomaly_positions) > 1:
            gaps = []
            for i in range(len(anomaly_positions) - 1):
                end_current = anomaly_positions[i][0] + anomaly_positions[i][1] - 1
                start_next = anomaly_positions[i + 1][0]
                gap = start_next - end_current - 1
                gaps.append(gap)
            
            self.logger.info(f"Gap statistics - Min: {min(gaps)}, Max: {max(gaps)}, Mean: {np.mean(gaps):.1f}")
        
        # Final validation
        self.logger.info(f"Final data types: {df[self.feature_cols + ['is_anomaly']].dtypes.to_dict()}")
        self.logger.info(f"is_anomaly unique values: {df['is_anomaly'].unique()}")
        
        return df


def main():
    """Main function with CLI"""
    logging.basicConfig(level=logging.INFO, format='%(levelname)s:%(name)s:%(message)s')
    
    parser = argparse.ArgumentParser(description='Inject extreme anomalies into weather dataset')
    parser.add_argument('--dataset', type=str, required=True, help='Path to original dataset.csv')
    parser.add_argument('--output', type=str, required=True, help='Output CSV file path')
    parser.add_argument('--n-anomalies', type=int, default=50, help='Number of anomalous events to inject')
    parser.add_argument('--min-duration', type=int, default=1, help='Minimum anomaly duration (samples)')
    parser.add_argument('--max-duration', type=int, default=10, help='Maximum anomaly duration (samples)')
    parser.add_argument('--min-gap', type=int, default=20, help='Minimum gap between anomalies (timesteps)')
    parser.add_argument('--seed', type=int, default=None, help='Random seed for reproducibility')
    
    args = parser.parse_args()
    
    # Create injector
    injector = ExtremeAnomalyInjector(args.dataset, seed=args.seed)
    
    # Inject anomalies
    df = injector.inject_anomalies(
        n_anomalies=args.n_anomalies,
        min_duration=args.min_duration,
        max_duration=args.max_duration,
        min_gap=args.min_gap
    )
    
    # Save to file
    df.to_csv(args.output, index=False)
    
    print(f"\n✓ Data with extreme anomalies saved to {args.output}")
    print(f"  Total samples: {len(df)}")
    print(f"  Anomalous samples: {df['is_anomaly'].sum()}")
    print(f"  Anomaly percentage: {df['is_anomaly'].sum()/len(df)*100:.2f}%")
    print(f"\nData quality checks:")
    print(f"  Any NaN values: {df.isna().any().any()}")
    print(f"  is_anomaly type: {df['is_anomaly'].dtype}")
    print(f"  is_anomaly values: {df['is_anomaly'].unique()}")
    print(f"\nFirst 10 rows:")
    print(df.head(10).to_string())
    print(f"\nFirst 10 anomalous rows:")
    anomalous = df[df['is_anomaly'] == 1].head(10)
    if len(anomalous) > 0:
        print(anomalous.to_string())
    else:
        print("No anomalous rows")


if __name__ == '__main__':
    main()